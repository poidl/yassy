...
// Formula: nt*(nppt-1)+1 as usize; nt is even, therefore N must be uneven
const N: usize = 4 * (2700 - 1) + 1;
...
pub fn blit_4t(fs: f64) -> [f64; N] {
    // Bandlimited impulse segment for sawtooth with BLIT (bandlimited
    // impulse train) approach. References:
    // Stilson, T. and Smith, J., 1996: Alias-free digital synthesis of
    // classic analog waveforms. Proc. International Computer Music
    // Conference
    // Frei, B.: Digital sound generation. Institute for Computer Music
    // and Sound Technology (ICST) Zurich University of the Arts.
    // See Frei's Fig. 17.
    // see also examples/blit.rs
    let nt: usize = 4;
    let nppt: usize = 2700;
    // N needs to be a constant if we want to stay in the heap memory
    // (2016/01/22)
